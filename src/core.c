...
QTEL_Status_t QTEL_CheckNetwork(QTEL_HandlerTypeDef *qtelPtr)
{
  ...
  if (AT_Check(&qtelPtr->atCmd, "+CREG", 4, respData) != AT_OK)
    return status;
  ...
  QTEL_Debug("Checking +CREG.... %d", qtelPtr->network_status);
  return status;
}
...
QTEL_Status_t QTEL_CheckGPRSNetwork(QTEL_HandlerTypeDef *qtelPtr)
{
  ...
  if (AT_Check(&qtelPtr->atCmd, "+CGREG", 4, respData) != AT_OK)
    return status;
...
QTEL_Status_t QTEL_ReqisterNetwork(QTEL_HandlerTypeDef *qtelPtr)
{
  ...
  if (AT_Check(&qtelPtr->atCmd, "+COPS", 1, respData) != AT_OK)
    goto endCmd;
  ...
  {
    // set
    if (AT_Command(&qtelPtr->atCmd, "+COPS", 1, paramData, 0, 0) != AT_OK)
      goto endCmd;

    // run
    if (AT_Command(&qtelPtr->atCmd, "+COPS", 0, 0, 0, 0) != AT_OK)
      goto endCmd;
  }
...
QTEL_Status_t QTEL_GetTime(QTEL_HandlerTypeDef *qtelPtr, QTEL_Datetime_t *dt)
{
  ...
  if (AT_Check(&qtelPtr->atCmd, "+CCLK", 1, respData) != AT_OK)
    return QTEL_ERROR;
...
QTEL_Status_t QTEL_CheckSugnal(QTEL_HandlerTypeDef *qtelPtr)
{
  ...
  if (AT_Command(&qtelPtr->atCmd, "+CSQ", 0, 0, 2, respData) != AT_OK)
    return QTEL_ERROR;
